...
const App: React.FC = () => {
  ...
  const buildPrompt = useCallback((opts: GenerationOptions, clothingText: string, hasClothingImg: boolean): string => {
    
    const promptLines: string[] = [
      'Generate a photorealistic portrait of the person from the provided image.',
      '**Crucial Core Objective: Preserve the person\'s facial identity with 100% accuracy. The generated face, including all features, structure, and unique characteristics, must be identical to the person in the source photo. The task is to change the photographic style (lighting, background, pose, clothing), not the person\'s identity.**'
    ];

    const settings: string[] = [];
    if (opts.style) settings.push(`- Style: The overall aesthetic should be '${opts.style}'.`);
    if (opts.lighting) settings.push(`- Lighting: Use '${opts.lighting}' to illuminate the subject.`);
    if (opts.background) settings.push(`- Background: Place the subject against a '${opts.background}' background.`);
    if (opts.angle) settings.push(`- Angle: The camera angle should be '${opts.angle}'.`);
    if (opts.composition) settings.push(`- Composition: The framing should be a '${opts.composition}'.`);
    if (opts.lens) settings.push(`- Lens: Captured with a '${opts.lens}' lens effect.`);
    if (opts.expression && opts.expression !== 'keep the original expression') {
      settings.push(`- Subject Details: The person's expression should be '${opts.expression}'.`);
    }
    if (opts.pose && opts.pose !== 'keep the original pose') {
      settings.push(` The person's pose should be '${opts.pose}'.`);
    }

    if (settings.length > 0) {
      promptLines.push('\nApply the following professional studio settings:');
      promptLines.push(...settings);
    }

    let clothingInstruction = '';
    if (hasClothingImg) {
      clothingInstruction = `The second image provided is a reference for the clothing and accessories. Dress the person in an outfit that is identical or very similar in style, color, and type to the one in the reference image.`;
    } else if (clothingText.trim() !== '') {
      clothingInstruction = `Dress the person in the following outfit: '${clothingText}'.`;
    }

    if (clothingInstruction) {
      promptLines.push(`- Clothing: ${clothingInstruction}`);
    }
    
    promptLines.push('- Execution: Automatically remove the original background and replace it. Perform professional-level retouching on skin and features, but maintain a natural, realistic look that honors the subject\'s appearance.');

    if (opts.style === 'ID photo') {
      promptLines.push("\n- Special Instructions for ID Photo: Adhere to standard passport photo requirements. This includes a neutral background (typically white or light gray), even lighting, a neutral or specified expression, and a forward-facing head position. Ensure appropriate headroom and composition for an official ID document.");
    }
  ...
  const handleGenerate = async () => {
    ...
    try {
      const prompt = buildPrompt(options, clothingPrompt, !!clothingImage);
